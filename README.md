...
本项目是一个基于 Qt Widgets、OpenCV 和 SQLite 的桌面端人脸识别系统。系统提供人员注册、图片人脸识别、摄像头实时检测与识别、人员列表展示和识别日志记录等功能，适合作为计算机视觉与桌面应用开发的综合实训项目。

当前版本采用 OpenCV Haar 级联分类器进行人脸检测，将裁剪后的人脸统一转换为 `64x64` 灰度图特征向量，并通过余弦相似度与数据库中已注册人员的人脸特征进行比对。该方案结构清晰、依赖简单，便于学习和展示完整的人脸识别流程。

## 已实现功能

- 人员注册：录入姓名、学号、部门，并选择一张人脸图片生成注册特征。
...
- `ui/RegisterDialog`：人员注册弹窗，收集姓名、学号、部门和注册图片路径。
...
- `storage/FaceRepository`：负责 SQLite 数据库连接、建表、人员信息、特征和日志读写。
...
2. 点击“注册人员”，填写姓名、学号和部门，并选择一张包含清晰正脸的人脸图片。
3. 注册成功后，人员信息会显示在右侧人员列表中。
4. 点击“导入图片”，选择待识别图片。
5. 点击“开始识别”，系统会检测图片中的人脸并显示识别姓名和相似度。
6. 点击“开始摄像头”，系统会读取默认摄像头并进行实时检测和识别。
7. 识别记录会显示在右侧日志区域，并写入本地 SQLite 数据库。
...
- `persons`：人员基本信息，包括姓名、学号、部门和创建时间。
- `face_features`：人员对应的人脸特征，特征向量以 JSON 数组形式保存。
...
- 支持同一人员录入多张图片，并对多组特征进行平均、聚类或质量筛选。
- 增加人员删除、人员编辑、单人详情查询、日志清空等更完整的数据管理功能。
